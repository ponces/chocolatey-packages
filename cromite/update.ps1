...
function global:au_GetLatest {
  $LatestRelease = Get-GitHubRelease uazo cromite

  return @{
    Version     = $LatestRelease.tag_name.TrimStart("v") -replace '\-.*', ''
    URL64       = $LatestRelease.assets | Where-Object {$_.name.EndsWith("-win.zip")} | Select-Object -ExpandProperty browser_download_url
